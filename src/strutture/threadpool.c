...
static void *workerpool_thread(void *threadpool) {    
	...
	pool->taskonthefly++;
        ...
        int r;
        if((r=pthread_cond_signal(&(pool->condFull))) != 0) {
...
int destroyThreadPool(threadpool_t *pool, int force) {    
    ...
    if (pthread_cond_broadcast(&(pool->condFull)) != 0) {
      UNLOCK_RETURN(&(pool->lock),-1);
      errno = EFAULT;
      return -1;
    }
...
int addToThreadPool(threadpool_t *pool, void (*f)(void *), void *arg) {
	...
	if (nopending) { 
	    ...
	    UNLOCK_RETURN(&(pool->lock),-1);
	    return 1;  // esco con valore "coda non presente" o "exiting"
	}
    }


    // in attesa di uno spazio libero nella coda, controllo spurious wakeups.
    while((pool->count >= queue_size) && (!pool->exiting)) {
        pthread_cond_wait(&(pool->condFull), &(pool->lock));
    }
