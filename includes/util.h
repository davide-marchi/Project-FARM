...
#define SYSCALL_EXIT_THREAD(name, r, sc, str, ...)	\
    if ((r=sc) == -1) {				\
	perror(#name);				\
	int errno_copy = errno;			\
	print_error(str, __VA_ARGS__);		\
	pthread_exit(&errno_copy);			\
    }

#define SYSCALL_CLEANUP_RETURN(name, r, sc, cleanup, str, ...)	\
    if ((r=sc) == -1) {				\
	perror(#name);				\
	int errno_copy = errno;			\
	print_error(str, __VA_ARGS__);		\
...
// Funzione per dormire tms millisecondi usando nanosleep
static inline int msleep(long tms) {
    struct timespec ts;
    int ret;
